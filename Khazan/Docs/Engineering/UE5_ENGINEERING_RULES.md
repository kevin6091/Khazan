...
## 2026-09-17 지속 가능한 최소 설계 사전 심사

- 이후 모든 Source 제안과 아키텍처 결정은 코드를 제시하기 전에 현재 실제 소비자, 이미 확정된 최종 게임 범위, 상태 소유자와 수명, 엔진·기존 프로젝트 기능으로 해결 가능한 범위를 먼저 대조한다. 이 대조 없이 새 class, interface, component, manager, subsystem, DataAsset, GameplayTag, delegate, handle 또는 wrapper를 추가하지 않는다.
- 선택 기준은 **확인된 완성 기능을 감당하는 구조 중 계층과 변경 지점이 가장 적은 것**이다. 단계별 마이그레이션은 기능 일부만 켤 수 있지만, 그 단계만 통과하는 시험 전용 public API나 곧 폐기할 상태 모델을 제품 구조로 만들지 않는다. probe와 임시 로그가 필요하면 임시임을 표시하고 제거 checkpoint를 같은 절차에 둔다.
- 재사용 가능성만으로 추상화하지 않는다. 새 타입은 독립 수명·cleanup, 서로 다른 구현을 다루는 실제 호출자, 또는 여러 소비자에서 이미 반복되고 같은 정책으로 유지해야 하는 계약 중 하나가 확인될 때 채택한다. 소비자가 둘이라는 숫자만으로 공통 base나 utility를 추출하지 않는다.
- 반대로 현재 한 edge만 구현한다는 이유로 이미 확인된 전체 기능의 의미를 지우지 않는다. 확정된 입력 종류, 콤보 단계, Root Motion, 취소와 해금처럼 곧 같은 실행 경로에서 쓰일 정보는 최소 표현으로 보존하고, 이후 단계에서는 규칙이나 데이터만 추가하도록 한다.
- 제안마다 `기존 엔진 기능 사용 → 현재 owner의 private 함수/지역 상태 → 기존 공통 타입 확장 → 새 추상화` 순서로 비교한다. 최종 설명에는 유지한 복잡성, 제거한 복잡성, 보류한 추상화와 각각의 실제 근거를 짧게 밝힌다.
