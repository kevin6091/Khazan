"""Build visual WildBoar coat Blueprints with source materials and idle playback.
...
REPORT = PROJECT / "Saved/ImportReports/WildBoar_AssemblyBuild_20260915.json"
CATALOG = META / "WildBoarAssemblyCatalog.json"
VERSION = "20260915_WildBoarVisualAssemblyV1"
...
def build_variant(row: dict, context: dict) -> dict:
    ...
    if EAL.does_asset_exist(path):
        blueprint = load(path)
        check(EAL.get_metadata_tag(blueprint, "WildBoarAssemblyVersion") == VERSION, "Unknown existing BP " + path)
        created = False
    else:
    ...
    tags = {
        ...
        "WildBoarLibraryVersion": MANIFEST["library_version"],
        "WildBoarAssemblyVersion": VERSION,
...
def main() -> None:
    ...
    try:
        ...
        check(set(context) == {"StormPass"}, "Expected the live StormPass source context")
        for row in MANIFEST["material_variants"]:
            record = build_variant(row, context)
            report["assemblies"].append(record)
            print("WILD_BOAR_ASSEMBLY", record["asset"], flush=True)
    ...
    print("WILD_BOAR_ASSEMBLY_BUILD_PASSED", flush=True)
