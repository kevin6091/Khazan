"""Build visual WildDog coat Blueprints with source materials and idle playback.
...
REPORT = PROJECT / "Saved/ImportReports/WildDog_AssemblyBuild_20260915.json"
CATALOG = META / "WildDogAssemblyCatalog.json"
VERSION = "20260915_WildDogVisualAssemblyV1"
...
def build_variant(row: dict, context: dict) -> dict:
    ...
    if EAL.does_asset_exist(path):
        blueprint = load(path)
        check(EAL.get_metadata_tag(blueprint, "WildDogAssemblyVersion") == VERSION, "Unknown existing BP " + path)
        created = False
    else:
    ...
    tags = {
        ...
        "WildDogLibraryVersion": MANIFEST["library_version"],
        "WildDogAssemblyVersion": VERSION,
...
def main() -> None:
    ...
    try:
        ...
        for row in MANIFEST["material_variants"]:
            ...
            print("WILD_DOG_ASSEMBLY", record["asset"], flush=True)
    ...
    print("WILD_DOG_ASSEMBLY_BUILD_PASSED", flush=True)
