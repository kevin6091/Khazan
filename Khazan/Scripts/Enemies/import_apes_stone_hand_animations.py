"""Create bounded batches of source-timed ApesStoneHandElite runtime AnimSequences."""
...
UPGRADABLE_VERSIONS = {"20260915_ApesStoneHandElite_SourceCompositePlaybackV1", VERSION}
...
def apply_sequence_contract(sequence, row: dict) -> None:
    ...
    EAL.set_metadata_tag(sequence, "ApesStoneHandEliteLibraryVersion", MANIFEST["library_version"])
...
def create_sequence(row: dict):
    ...
    if EAL.does_asset_exist(path):
        ...
        if EAL.get_metadata_tag(sequence, "EnemyTimingVersion") not in UPGRADABLE_VERSIONS:
            raise RuntimeError("Unrecognized existing ApesStoneHandElite animation " + path)
    ...
    controller.open_bracket("Import source-timed ApesStoneHandElite playback", False)
...
def command_range() -> tuple[int, int]:
    ...
    for part in unreal.SystemLibrary.get_command_line().split():
        if part.startswith("-ApesStoneHandEliteStart="):
            start = int(part.split("=", 1)[1])
        elif part.startswith("-ApesStoneHandEliteCount="):
...
def main() -> None:
    ...
    target = PROJECT / "Saved/ImportReports/ApesStoneHandEliteAnimationBatches" / f"{start:04d}.json"
    ...
    try:
        for index, row in enumerate(ROWS[start : start + count], start):
            ...
            write(target, report)
            print("APES_STONE_HAND_ANIMATION", index + 1, "/", len(ROWS), row["destination"], flush=True)
