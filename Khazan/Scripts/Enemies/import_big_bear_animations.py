"""Create bounded batches of source-timed BigBear runtime AnimSequences."""
...
UPGRADABLE_VERSIONS = {"20260910_BigBear_SourceCompositePlaybackV1", VERSION}
...
def apply_sequence_contract(sequence, row: dict) -> None:
    ...
    EAL.set_metadata_tag(sequence, "BigBearLibraryVersion", MANIFEST["library_version"])
...
def create_sequence(row: dict):
    ...
    if EAL.does_asset_exist(path):
        ...
        if EAL.get_metadata_tag(sequence, "EnemyTimingVersion") not in UPGRADABLE_VERSIONS:
            raise RuntimeError("Unrecognized existing BigBear animation " + path)
    ...
    controller.open_bracket("Import source-timed BigBear playback", False)
...
def command_range() -> tuple[int, int]:
    ...
    for part in unreal.SystemLibrary.get_command_line().split():
        if part.startswith("-BigBearStart="):
            start = int(part.split("=", 1)[1])
        elif part.startswith("-BigBearCount="):
...
def main() -> None:
    ...
    target = PROJECT / "Saved/ImportReports/BigBearAnimationBatches" / f"{start:04d}.json"
    ...
    try:
        for index, row in enumerate(ROWS[start : start + count], start):
            ...
            write(target, report)
            print("BIG_BEAR_ANIMATION", index + 1, "/", len(ROWS), row["destination"], flush=True)
