"""Real-RHI shader and saved WildBoar catalogue validation."""
...
META = PROJECT / "Content/_Art/Enemies/Shared/Beasts/WildBoar/Metadata/Extraction_20260915"
...
REPORT = PROJECT / "Saved/ImportReports/WildBoar_RenderAudit_20260915.json"
SUMMARY = META / "RenderValidationSummary.json"
MAP_PATH = MANIFEST["destination_root"] + "/Preview/L_EN_WildBoar_Catalogue"
...
def capture_preview(report):
    ...
    position = unreal.Vector(1200.0, 500.0, 300.0)
    target = unreal.Vector(0.0, 500.0, 120.0)
    ...
    unreal.RenderingLibrary.export_render_target(world, texture, str(folder), "WildBoar_RenderPreview_20260915.png")
    path = folder / "WildBoar_RenderPreview_20260915.png"
    ...
    component.capture_scene()
    unreal.RenderingLibrary.export_render_target(world, texture, str(folder), "WildBoar_BaseColorPreview_20260915.png")
    report["base_color_capture"] = str(folder / "WildBoar_BaseColorPreview_20260915.png")
    if "-WildBoarDiagnostic" in unreal.SystemLibrary.get_command_line():
        ...
        component.set_editor_property("capture_source", unreal.SceneCaptureSource.SCS_FINAL_COLOR_LDR)
        component.capture_scene()
        unreal.RenderingLibrary.export_render_target(world, texture, str(folder), "WildBoar_DiagnosticOpaque_20260915.png")
    report["preview_capture_settings"] = {"status":"Assistant-selected presentation settings; not source gameplay metadata", "resolution":[1200,1000], "fov_degrees":55.0, "camera_cm":[1200,500,300], "target_cm":[0,500,120], "render_target_format":"RTF_RGBA8_SRGB"}
...
def main() -> None:
    ...
    report["capture_context"] = "EditorAfterWarmup" if "capture_wild_boar_editor_preview.py" in command_line else "ImmediateCommandletCapture"
    try:
        ...
        material_paths = [
            MANIFEST["destination_root"] + "/Materials/Masters/M_EN_WildBoar_SurfacePreview",
            MANIFEST["destination_root"] + "/Materials/Masters/M_EN_WildBoar_EyePreview",
        ...
        for path in material_paths:
            ...
            print("WILD_BOAR_SHADER_OK", path, flush=True)
        ...
        if not current_world or current_world.get_path_name().split(".", 1)[0] != MAP_PATH:
            check(level_subsystem.load_level(MAP_PATH), "Unable to reload saved WildBoar catalogue")
        actor_map = {
    ...
    print("WILD_BOAR_RENDER_AUDIT_PASSED", flush=True)
