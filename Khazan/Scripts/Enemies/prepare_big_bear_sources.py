"""Build the source-backed BigBear_E art and animation closure.

The two requested levels use the same CB_BigBear_E character.  This script
starts from their exact spawn handlers and follows only BigBear character,
...
EXTERNAL = FMODEL_ROOT / "EnemyExtracts/BigBear_20260910"
...
LEVEL_SOURCES = {
    ...
    "StormPass": FMODEL_ROOT
    / "Exports/BBQ/Content/_Kazan_/Level/StormPass/StormPass_Spawn_Main01.json",
}

BASE_ROOT = "BBQ/Content/_Kazan_/Design/Monster/Beast/BigBear_E/Base_Setting/"
DESIGN_ROOT = "BBQ/Content/_Kazan_/Design/Monster/Beast/BigBear_E/"
MODEL_ROOT = "BBQ/Content/_Kazan_/Art/Character/CHA_Model/Monster/BigBear/"
RECIPE_ROOT = "BBQ/Content/_Kazan_/Art/Character/CHA_Data/RandomLookInfo/MonsterType/CD_RD_M_BigBear"
MATERIAL_ROOT = "BBQ/Content/_Kazan_/Art/Character/CHA_Material/"
SELECTED_MESHES = {
    MODEL_ROOT + "Model/C_M_BigBear",
    MODEL_ROOT + "Model/C_M_BigBear_EmptyMesh",
}
...
def source_spawn_records() -> tuple[list[dict], set[str]]:
    ...
    for level, source in LEVEL_SOURCES.items():
        ...
        matches = [
            ...
            and "BigBear" in json.dumps(value, ensure_ascii=False)
        ]
        if not matches:
            raise RuntimeError(f"No BigBear spawn handler in {source}")
...
def should_follow(owner_package: str, owner_type: str, target: str) -> bool:
    ...
    if target.startswith(MATERIAL_ROOT):
        return owner_type in {
            ...
            "Texture2D",
        } or owner_type.startswith("CD_RD_M_BigBear") or owner_package.startswith(
...
def discover() -> None:
    ...
    while pending:
        round_number += 1
        objects, round_failures = ensure_metadata(pending, f"BigBear_Metadata_{round_number:02d}")
    ...
    result = {
        ...
        "selection": {
            "mesh": "C_M_BigBear from CD_RD_M_BigBear_001.FacePartsList[0].Mesh",
            "material_variations": "All three source MaterialVariations on the selected mesh",
            "animations": "Composite/direct sequence references reachable from the two level spawns and CB_BigBear_E gameplay metadata",
        },
    }
    ...
    write(
        ROOT / "DiscoverySummary.json",
        {
            ...
            "indexed_big_bear_packages": len(indexed),
...
def export_sources() -> None:
    ...
    run_exporter("export", ROOT / "Assets", export_packages, "BigBear_ArtAnimationExport")
    run_exporter("raw", EXTERNAL / "RawCookedArchive", closure["packages"], "BigBear_RawArchive")
...
def main() -> None:
    ...
    if mode == "discover":
        ...
        raise SystemExit("Usage: prepare_big_bear_sources.py [discover|export]")
