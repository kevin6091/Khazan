"""Build a visual ApesStoneHandElite Early/Standard Blueprints with source materials and idle playback.

The Actor has an EnemyBody SkeletalMeshComponent. Enemy AI, attacks and damage
remain outside this art import; original gameplay data is preserved as metadata.
"""
...
REPORT = PROJECT / "Saved/ImportReports/ApesStoneHandElite_AssemblyBuild_20260915.json"
CATALOG = META / "ApesStoneHandEliteAssemblyCatalog.json"
VERSION = "20260915_ApesStoneHandEliteVisualAssemblyV1"
...
def build_variant(row: dict, context: dict) -> dict:
    ...
    if EAL.does_asset_exist(path):
        blueprint = load(path)
        check(EAL.get_metadata_tag(blueprint, "ApesStoneHandEliteAssemblyVersion") == VERSION, "Unknown existing BP " + path)
        created = False
    else:
    ...
    tags = {
        ...
        "EnemySpecies": "ApesStoneHandElite",
        "EnemySourceLevels": json.dumps(MANIFEST["source_levels"], ensure_ascii=False),
        "OriginalPackage": row["source_character"],
        "SourceEffectiveRecipe": json.dumps(row["effective_recipe"]),
        "SourceCharacterScale": str(row["source_character_scale"]),
        "BoneModCoverage": "Source preset metadata retained; native xxBoneModPreset axis/zero semantics are not implemented by this visual Actor.",
        "ApesStoneHandEliteLibraryVersion": MANIFEST["library_version"],
        "ApesStoneHandEliteAssemblyVersion": VERSION,
    ...
    bone_mods = []
    for reference in row["effective_recipe"].get("BoneModRandomPreset", []):
        source = package_path(reference)
        bone_mods.append({"source_package": source, "source_objects": json.loads((ROOT / "Metadata" / (source + ".json")).read_text(encoding="utf-8"))})
    tags["SourceBoneModPresetsJSON"] = json.dumps(bone_mods)
...
def main() -> None:
    ...
    try:
        ...
        check(set(context) == {"HeinMach", "StormPass"}, "Expected source level contexts")
        for row in MANIFEST["material_variants"]:
            record = build_variant(row, context)
            report["assemblies"].append(record)
            print("APES_STONE_HAND_ASSEMBLY", record["asset"], flush=True)
    ...
    print("APES_STONE_HAND_ASSEMBLY_BUILD_PASSED", flush=True)
