"""Create the canonical UE import manifest for live StormPass WildDog looks."""
...
META = ROOT / "Metadata"
DEST = "/Game/_Art/Enemies/Shared/Beasts/WildDog"
PROJECT_META = PROJECT / "Content/_Art/Enemies/Shared/Beasts/WildDog/Metadata/Extraction_20260915"

CB = "BBQ/Content/_Kazan_/Design/Monster/Beast/WhiteDog/Base_Setting/CB_WhiteDog"
AP = "BBQ/Content/_Kazan_/Design/Monster/Beast/WhiteDog/Base_Setting/AP_WhiteDog"
BODY = "BBQ/Content/_Kazan_/Art/Character/CHA_Model/Monster/PicaroonDog/Model/C_M_WildDog"
...
def derive_full_animation_skeleton_psk(body_file: pathlib.Path, animation_files: list[pathlib.Path]):
    ...
    if len(layouts) != 1:
        raise RuntimeError(f"WildDog animation skeleton layouts differ: {[len(value) for value in layouts]}")
    ...
    if not set(mesh_names).issubset(animation_names):
        raise RuntimeError("Mesh contains bones absent from the source animation skeleton")
    name_to_animation = {name: index for index, name in enumerate(animation_names)}
    name_to_mesh = {name: index for index, name in enumerate(mesh_names)}
    remap = {index: name_to_animation[name] for index, name in enumerate(mesh_names)}
    ...
    for index in range(ref_count):
        ...
        animation_index = remap[index]
        animation_record = animation_payload[animation_index * animation_size : (animation_index + 1) * animation_size]
        mesh_parent = struct.unpack_from("<i", mesh_record, 72)[0]
        animation_parent = struct.unpack_from("<i", animation_record, 72)[0]
        if (remap[mesh_parent] if mesh_parent >= 0 else -1) != animation_parent:
        ...
        if index == 0:
            converted[3] *= -1.0
        reference_differences.append({"bone": mesh_names[index], "mesh_bind_transform": mesh_transform,
            "animation_skeleton_reference_in_psk_space": converted,
            "maximum_component_difference": max(abs(left-right) for left,right in zip(mesh_transform,converted))})

    extended = bytearray()
    parent_indices = [struct.unpack_from("<i", animation_payload, index*animation_size+72)[0] for index in range(animation_count)]
    for index, name in enumerate(animation_names):
        if name in name_to_mesh:
            old_index = name_to_mesh[name]
            record = bytearray(ref_payload[old_index*ref_size:(old_index+1)*ref_size])
        else:
        ...
        struct.pack_into("<ii", record, 68, parent_indices.count(index), parent_indices[index])
        extended.extend(record)

    body_chunks[ref_index] = (ref_tag, ref_flags, ref_size, animation_count, bytes(extended))
    weight_index = next(i for i,c in enumerate(body_chunks) if c[0].startswith(b"RAWWEIGHTS"))
    tag, flags, size, count, payload = body_chunks[weight_index]
    if size != 12:
        raise RuntimeError("Unexpected weight record")
    weights = bytearray(payload)
    for index in range(count):
        offset = index*size+8
        old_bone = struct.unpack_from("<i", payload, offset)[0]
        struct.pack_into("<i", weights, offset, remap[old_bone])
    body_chunks[weight_index] = (tag, flags, size, count, bytes(weights))
    ...
    return {
        ...
        "added_unweighted_animation_bones": [name for name in animation_names if name not in name_to_mesh],
        "derivation": "Source mesh bind transforms preserved by bone name; hierarchy/weight bone indices remapped to the verified full PSA order. Missing unweighted bones use converted PSA reference transforms. Geometry, UVs, colors and numerical skin weights unchanged.",
        "bone_index_remap": remap, "shared_reference_comparison": reference_differences,
...
def main():
    ...
    usage = read(ROOT / "VariantUsage.json")
    selected_variants = usage["variants"]
    if not package(character_mesh["Properties"]["SkeletalMesh"]).endswith("_EmptyMesh"):
        raise RuntimeError("Source modular carrier changed")
    mesh_rows = []
    for variant in selected_variants:
        mesh_package = variant["mesh"]
        suffix = "WildDog"
        ...
        if mesh_rows:
            previous = actorx_chunks(pathlib.Path(mesh_rows[0]["source_file"]))
            current = actorx_chunks(file)
            comparable = lambda chunks: [c for c in chunks if not c[0].startswith(b"MATT0000")]
            if comparable(previous) != comparable(current):
                raise RuntimeError("Variant geometry/bind differs; import distinct mesh before sharing")
            mesh_rows[0]["source_mesh_aliases"].append(mesh_package)
            continue
        mesh_rows.append({
            "source_package": mesh_package, "source_file": str(file),
            "source_mesh_aliases": [mesh_package],
            "destination": f"{DEST}/Meshes/SK_EN_{suffix}",
            "source_skeleton": package(obj["Properties"]["Skeleton"]),
            "skeleton_destination": f"{DEST}/Skeletons/SKEL_EN_WildDog",
    ...
    skeleton_mesh = {}
    for m in mesh_rows:
        skeleton_mesh.setdefault(m["source_skeleton"], m)
    ...
    for p, ts in sorted(types.items()):
        for kind, target in [("AnimSequence", source_anims), ("AnimComposite", composites)]:
            ...
            if skeleton not in skeleton_mesh:
                excluded.append({"source": p, "skeleton": skeleton, "reason": "Referenced animation uses another character or helper skeleton; retain metadata without importing it into the WildDog skeleton."})
                continue
    ...
    leaves = {p for v in selected_variants for p in v["materials"]}
    ...
    for p in sorted(leaves):
        category = "Eye" if "_Eye" in p else "Surface"
    ...
    for p, obj in hierarchy.items():
        ...
        rows.append({
            "source_package": p, "destination": f"{DEST}/Materials/Instances/MI_EN_WildDog_" + p.rsplit("/", 1)[1],
    ...
    for p in sorted(required_textures):
        ...
        textures.append({
            ...
            "destination": f"{DEST}/Textures/" + unique_name("T_EN_WildDog_", p, tex_names),
    ...
    variants = []
    for index, selection in enumerate(selected_variants):
        mesh = next(m for m in mesh_rows if selection["mesh"] in m["source_mesh_aliases"])
        variants.append({
            "variant": index + 1, "role": selection["role"], "source_material_index": selection["material_index"],
            "source_materials": selection["materials"], "materials": [material_map[p] for p in selection["materials"]],
            "blueprint": DEST + "/Blueprints/BP_EN_WildDog_" + selection["role"], "mesh": mesh["destination"],
            "idle_source": idle, "idle_asset": anim_map[idle], "source_character": selection["source_character"],
            "source_recipe": selection["source_recipe"], "effective_recipe": selection["effective_recipe"],
            "source_character_scale": selection["source_character_properties"].get("Scale", 1.0),
            "source_character_scale_provenance": "serialized CB Scale" if "Scale" in selection["source_character_properties"] else "presentation fallback 1.0; CB metadata has no serialized Scale",
        })
    result = {
        "schema_version": 1, "library_version": "20260915_WildDogV1", "destination_root": DEST,
        "source_character": CB, "source_animation_profile": AP, "source_levels": ["StormPass"],
        "inspected_levels": ["HeinMach", "StormPass"],
        ...
        "limitations": [
            ...
            "Physics, cloth, postprocess control rigs, proprietary notify behavior, VFX/audio and gameplay are source metadata/cooked archives; preview Actor does not implement enemy combat.",
    ...
    for name in ["SourceClosure.json","LevelPresence.json","DiscoverySummary.json","SourceExportSummary.json","VariantUsage.json"]:
