"""Restore the WildBoar enemy render library from cooked metadata and exports.
...
SKELETON_VERSION = "20260915_WildBoar_SourceSkeletonV1"
REPORT = PROJECT / "Saved/ImportReports/WildBoar_LibraryBuild_20260915.json"
BASELINE = PROJECT / "Saved/ImportReports/WildBoar_ProtectedBaseline_20260915.json"
...
def clear_asset_directory(path: str) -> list[str]:
    if not path.startswith(DEST + "/_ImportStaging/"):
        raise RuntimeError("Refuse cleanup outside new WildBoar import staging: " + path)
...
def set_common_metadata(asset, source_package: str, role: str) -> None:
    ...
    EAL.set_metadata_tag(asset, "WildBoarLibraryVersion", VERSION)


def new_asset(path: str, cls, factory):
    if EAL.does_asset_exist(path):
        asset = load(path)
        if EAL.get_metadata_tag(asset, "WildBoarLibraryVersion") != VERSION:
            raise RuntimeError("Unrecognized existing WildBoar asset " + path)
        return asset, False
...
def import_one(source: str, destination: str, factory):
    if EAL.does_asset_exist(destination):
        ...
        if EAL.get_metadata_tag(asset, "WildBoarLibraryVersion") != VERSION:
            raise RuntimeError("Unrecognized existing WildBoar import " + destination)
        return asset, False
...
def import_textures(report: dict) -> dict[str, object]:
    ...
    for index, row in enumerate(MANIFEST["textures"], 1):
        ...
        if index % 10 == 0 or index == len(MANIFEST["textures"]):
            print("WILD_BOAR_TEXTURE", index, "/", len(MANIFEST["textures"]), flush=True)
    return mapping
...
def build_master(category: str, texture_map: dict[str, object], report: dict):
    name = "M_EN_WildBoar_" + category + "Preview"
    path = f"{DEST}/Materials/Masters/{name}"
    rebuilt = False
    if EAL.does_asset_exist(path):
        existing = load(path)
        if EAL.get_metadata_tag(existing, "WildBoarMaterialAdapterVersion") != MATERIAL_ADAPTER_VERSION:
            if EAL.get_metadata_tag(existing, "WildBoarLibraryVersion") != VERSION:
    ...
    EAL.set_metadata_tag(master, "WildBoarMaterialAdapterVersion", MATERIAL_ADAPTER_VERSION)
...
def import_mesh(row, material_map, report):
    ...
    if EAL.does_asset_exist(destination):
        ...
        if EAL.get_metadata_tag(mesh, "WildBoarLibraryVersion") != VERSION:
            raise RuntimeError("Unrecognized existing WildBoar mesh " + destination)
        created = False
    else:
    ...
    for asset in [mesh, skeleton]:
        EAL.set_metadata_tag(asset, "WildBoarSkeletonVersion", SKELETON_VERSION)
    ...
    print("WILD_BOAR_MESH_IMPORTED", destination, flush=True)
...
def main() -> None:
    ...
    try:
        ...
        write(REPORT, report)
    print("WILD_BOAR_LIBRARY_BUILD_PASSED", flush=True)
