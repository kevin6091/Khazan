"""Create the canonical UE import manifest for the ApesStoneHandElite Early/Standard enemies."""
...
PROJECT_META = PROJECT / "Content/_Art/Enemies/Shared/Elites/ApesStoneHandElite/Metadata/Extraction_20260915"

CB = "BBQ/Content/_Kazan_/Design/Monster/Apes/ApesStoneHand_E/Base_Setting/CB_ApesStoneHand_E_Early"
AP = CB.rsplit("/", 1)[0] + "/AP_ApesStoneHand_E"
BODY = "BBQ/Content/_Kazan_/Art/Character/CHA_Model/Monster/ApesStoneHandElite/Model/C_M_ApesStoneHandElite"
...
def derive_full_animation_skeleton_psk(body_file: pathlib.Path, animation_files: list[pathlib.Path]):
    ...
    if len(layouts) != 1:
        raise RuntimeError(f"ApesStoneHandElite animation skeleton layouts differ: {[len(value) for value in layouts]}")
...
def main():
    ...
    for variant in selected_variants:
        ...
        mesh_rows.append({
            ...
            "skeleton_destination": f"{DEST}/Skeletons/SKEL_EN_ApesStoneHandElite",
    ...
    for p, ts in sorted(types.items()):
        for kind, target in [("AnimSequence", source_anims), ("AnimComposite", composites)]:
            ...
            if skeleton not in skeleton_mesh:
                excluded.append({"source": p, "skeleton": skeleton, "reason": "Shared missile gameplay carrier; not the ApesStoneHandElite body skeleton."})
                continue
    ...
    for p, obj in hierarchy.items():
        ...
        rows.append({
            "source_package": p, "destination": f"{DEST}/Materials/Instances/MI_EN_ApesStoneHandElite_" + p.rsplit("/", 1)[1],
    ...
    for p in sorted(required_textures):
        ...
        textures.append({
            ...
            "destination": f"{DEST}/Textures/" + unique_name("T_EN_ApesStoneHandElite_", p, tex_names),
    ...
    for index, selection in enumerate(selected_variants):
        ...
        variants.append({
            ...
            "blueprint": DEST + "/Blueprints/BP_EN_ApesStoneHandElite_" + selection["role"], "mesh": mesh["destination"],
            ...
            "source_character_scale": selection["source_character_properties"]["Scale"],
        })
    result = {
        "schema_version": 1, "library_version": "20260915_ApesStoneHandEliteV1", "destination_root": DEST,
        "source_character": CB, "source_animation_profile": AP, "source_levels": ["HeinMach", "StormPass"],
