"""Finalize the WildBoar source archive without copying any game key."""
...
ARCHIVE = pathlib.Path.home() / "Desktop" / "카잔" / "EnemyExtracts" / "WildBoar_20260915"
README = PROJECT / "Docs/Art/WILD_BOAR_EXTRACTION_2026-09-15.md"
SCRIPT_NAMES = [
    "prepare_wild_boar_sources.py",
    "build_wild_boar_import_manifest.py",
    "prepare_wild_boar_animation_timing.py",
    "import_wild_boar_library.py",
    "import_wild_boar_animations.py",
    "run_wild_boar_animation_batches.py",
    "build_wild_boar_assemblies.py",
    "audit_wild_boar_library.py",
    "verify_wild_boar_render.py",
    "capture_wild_boar_editor_preview.py",
    "archive_wild_boar_sources.py",
    "run_wild_boar_editor_stage.py",
    "audit_wild_boar_sources.py",
...
def main() -> None:
    ...
    if validation["status"] != "passed" or render["status"] != "passed":
        raise RuntimeError("WildBoar final validation has not passed")
    if "capture_wild_boar_editor_preview.py" not in render.get("command_line", ""):
    ...
    for stage in ["library", "assemblies", "audit", "render"]:
        process = read(REPORTS / f"WildBoar_Process_{stage}_20260915.json")
    ...
    for name in ["WildBoar_RenderPreview_20260915.png", "WildBoar_BaseColorPreview_20260915.png"]:
    ...
    with zipfile.ZipFile(META / "SourceMetadata.zip", "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as bundle:
        # Classification evidence for WildBoar mesh aliases and helper assets is
        # also preserved as JSON, without duplicating visible geometry in UE.
    ...
    final = {
        ...
        "scope": "Live StormPass CB_WildBoar_New coat outcomes and source-timed playback library.",
    ...
    for destination in [
        ...
        REPORTS / "WildBoar_FinalExtraction_20260915.json",
    ]:
