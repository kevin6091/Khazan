"""Build a visual Yetuga boss Blueprint with source materials and idle playback.

The Actor has an EnemyBody SkeletalMeshComponent. Boss AI, attacks and damage
...
REPORT = PROJECT / "Saved/ImportReports/Yetuga_AssemblyBuild_20260911.json"
CATALOG = META / "YetugaAssemblyCatalog.json"
VERSION = "20260911_YetugaVisualAssemblyV1"
...
def build_variant(row: dict, context: dict) -> dict:
    ...
    if EAL.does_asset_exist(path):
        blueprint = load(path)
        check(EAL.get_metadata_tag(blueprint, "YetugaAssemblyVersion") == VERSION, "Unknown existing BP " + path)
        created = False
    else:
    ...
    tags = {
        ...
        "YetugaLibraryVersion": MANIFEST["library_version"],
        "YetugaAssemblyVersion": VERSION,
...
def main() -> None:
    ...
    try:
        ...
        check(set(context) == {"HeinMach"}, "Expected HeinMach source context")
        for row in MANIFEST["material_variants"]:
            record = build_variant(row, context)
            report["assemblies"].append(record)
            print("YETUGA_ASSEMBLY", record["asset"], flush=True)
    ...
    print("YETUGA_ASSEMBLY_BUILD_PASSED", flush=True)
