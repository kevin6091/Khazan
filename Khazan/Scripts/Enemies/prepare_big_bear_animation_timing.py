"""Bake source-authored BigBear composite timing into runtime playback poses.
...
def main() -> None:
    ...
    for index, row in enumerate(manifest["composites"]):
            ...
            skipped.append(
                {
                    "source": source_package,
                    "inputs": unsupported,
                    "reason": "Composite references a non-BigBear or unsupported animation asset; metadata retained and no incompatible skeleton assigned.",
                }
            )
            continue
        ...
        target_rate = 1 / curve["TimePerFrame"] if active else max(rates or [30.0])
        ...
        output = {
            ...
            "target_sample_rate_source": "DilationCurve.TimePerFrame" if active else "Referenced sequence NumFrames/SequenceLength",
            "segments": properties["AnimationTrack"]["AnimSegments"],
        ...
        if index % 20 == 0:
            print("BIG_BEAR_BAKE", index, "/", len(manifest["composites"]), flush=True)
