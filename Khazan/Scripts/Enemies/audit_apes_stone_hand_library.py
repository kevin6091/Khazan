"""Fresh-process audit and preview catalogue for the shared ApesStoneHandElite library."""
...
CONTENT_ROOT = PROJECT / "Content/_Art/Enemies/Shared/Elites/ApesStoneHandElite"
META = CONTENT_ROOT / "Metadata/Extraction_20260915"
...
CATALOG = json.loads((META / "ApesStoneHandEliteAssemblyCatalog.json").read_text(encoding="utf-8"))
EVENTS = json.loads((META / "PlaybackEventTimes.json").read_text(encoding="utf-8"))["events"]
ANIMATION_IMPORT_AUDIT = json.loads(
    (PROJECT / "Saved/ImportReports/ApesStoneHandElite_AnimationImportAudit_20260915.json").read_text(encoding="utf-8")
)
...
MAP_PATH = DEST + "/Preview/L_EN_ApesStoneHandElite_Catalogue"
REPORT = PROJECT / "Saved/ImportReports/ApesStoneHandElite_FreshAudit_20260915.json"
SUMMARY = META / "ValidationSummary.json"
TIMING_VERSION = "20260915_ApesStoneHandElite_SourceCompositePlaybackV2_RootLockPropagation"
ASSEMBLY_VERSION = "20260915_ApesStoneHandEliteVisualAssemblyV1"
MATERIAL_ADAPTER_VERSION = "20260915_ApesStoneHandElitePreviewV4_BoundedParameterRetention"
...
def build_catalogue(report: dict) -> None:
    if EAL.does_asset_exist(MAP_PATH):
        check(LEVELS.load_level(MAP_PATH), "Unable to load ApesStoneHandElite catalogue")
    else:
        check(LEVELS.new_level(MAP_PATH), "Unable to create ApesStoneHandElite catalogue")
    ...
    for index, row in enumerate(CATALOG):
        ...
        actor.set_folder_path("EnemyCatalogue/ApesStoneHandElite")

    if "ApesStoneHandEliteCatalogue_KeyLight" not in existing:
        key = ACTORS.spawn_actor_from_class(
            unreal.DirectionalLight,
            unreal.Vector(-300.0, 0.0, 700.0),
            unreal.Rotator(pitch=-40.0, yaw=-35.0, roll=0.0),
        )
        key.set_actor_label("ApesStoneHandEliteCatalogue_KeyLight")
        ...
        fill.set_actor_label("ApesStoneHandEliteCatalogue_FillLight")
        ...
        floor.set_actor_label("ApesStoneHandEliteCatalogue_Floor")
        ...
        camera.set_actor_label("ApesStoneHandEliteCatalogue_Camera")
    ...
    check(LEVELS.save_current_level(), "Unable to save ApesStoneHandElite catalogue")
...
def verify_materials(report: dict) -> None:
    ...
    master_paths = {
        "Surface": DEST + "/Materials/Masters/M_EN_ApesStoneHandElite_SurfacePreview",
        "Eye": DEST + "/Materials/Masters/M_EN_ApesStoneHandElite_EyePreview",
    }
    for path in master_paths.values():
        master = load(path)
        check(isinstance(master, unreal.Material), "Preview master type mismatch " + path)
        check(EAL.get_metadata_tag(master, "ApesStoneHandEliteMaterialAdapterVersion") == MATERIAL_ADAPTER_VERSION, "Master version " + path)
        check(
    ...
    for row in MANIFEST["materials"]:
        ...
        if "BlendMode" in source_base:
            check(actual_base.get_editor_property("override_blend_mode") and actual_base.get_editor_property("blend_mode") == unreal.BlendMode.BLEND_MASKED, "Source masked blend override mismatch")
        if "DitheredLODTransition" in source_base:
            check(actual_base.get_editor_property("override_dithered_lod_transition") and actual_base.get_editor_property("dithered_lod_transition") == source_base["DitheredLODTransition"], "Dithered LOD source override mismatch")
...
def verify_animations(report: dict, mesh, skeleton) -> None:
    ...
    for index, row in enumerate(ANIMATIONS, 1):
        ...
        if index % 20 == 0 or index == len(ANIMATIONS):
            print("APES_STONE_HAND_AUDIT_ANIMATION", index, "/", len(ANIMATIONS), flush=True)
...
def verify_assemblies(report: dict, mesh) -> None:
    check(len(CATALOG) == len(MANIFEST["material_variants"]) == 2, "Assembly variant count mismatch")
    for catalog_row, source_row in zip(CATALOG, MANIFEST["material_variants"]):
        ...
        check(EAL.get_metadata_tag(blueprint, "EnemySpecies") == "ApesStoneHandElite", "Blueprint species mismatch")
        check(EAL.get_metadata_tag(blueprint, "ApesStoneHandEliteAssemblyVersion") == ASSEMBLY_VERSION, "Blueprint version mismatch")
    ...
    for row in MANIFEST["material_variants"]:
        ...
        check(len(components) == 1, "Visual ApesStoneHandElite BP must have exactly one skeletal component " + label)
...
def verify_structure(report: dict) -> None:
    expected = {
        ...
        SKELETON_PATH,
        MAP_PATH,
        DEST + "/Materials/Masters/M_EN_ApesStoneHandElite_SurfacePreview",
        DEST + "/Materials/Masters/M_EN_ApesStoneHandElite_EyePreview",
    }
    ...
    check(actual == expected, "ApesStoneHandElite asset set mismatch: missing=" + repr(expected-actual) + "; extra=" + repr(actual-expected))
    orphan_files = [
...
def main() -> None:
    ...
    print("APES_STONE_HAND_FRESH_AUDIT_PASSED", flush=True)
