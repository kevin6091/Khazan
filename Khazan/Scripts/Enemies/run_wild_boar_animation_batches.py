"""Run checkpointed WildBoar animation imports in isolated Editor processes."""
...
BATCH_SIZE = 12
VERSION = "20260915_WildBoar_SourceCompositePlaybackV2_RootLockPropagation"
...
def main() -> None:
    ...
    for start in range(0, min(limit, len(ROWS)), BATCH_SIZE):
        ...
        report_path = PROJECT / "Saved/ImportReports/WildBoarAnimationBatches" / f"{start:04d}.json"
        ...
        if (
            ...
            continue
        log = PROJECT / "Saved/Logs" / f"WildBoarAnimations_{start:04d}_20260915.log"
        stdout = log.with_suffix(".stdout.txt")
        args = [
            str(ENGINE),
            str(PROJECT / "Khazan.uproject"),
            "-run=pythonscript",
            "-script=" + str(PROJECT / "Scripts/Enemies/import_wild_boar_animations.py"),
            f"-WildBoarStart={start}",
            f"-WildBoarCount={count}",
            "-unattended",
            "-nullrhi",
            "-nosound",
            "-nosplash",
        ...
        if process.returncode or data.get("status") != "passed" or len(data.get("assets", [])) != count:
            print("WILD_BOAR_BATCH_FAILED", start, process.returncode, data.get("error"), flush=True)
            raise SystemExit(1)
        reports.append(data)
        print(
            "WILD_BOAR_BATCH_PASSED",
            start,
            count,
            "total",
    ...
    if limit >= len(ROWS):
        ...
        result = {
            ...
            "batch_reports": [
                str(PROJECT / "Saved/ImportReports/WildBoarAnimationBatches" / f"{report['start']:04d}.json")
                for report in reports
            ],
        }
        target = PROJECT / "Saved/ImportReports/WildBoar_AnimationImportAudit_20260915.json"
