"""Bake source-authored WildDog composite timing into runtime playback poses.
...
def main() -> None:
    ...
    for index, row in enumerate(manifest["composites"]):
            ...
            skipped.append(
                {
                    "source": source_package,
                    "inputs": unsupported,
                    "reason": "Composite references a non-WildDog or unsupported animation asset; metadata retained and no incompatible skeleton assigned.",
                }
            )
            continue
        ...
        if index % 20 == 0:
            print("WILD_DOG_BAKE", index, "/", len(manifest["composites"]), flush=True)
