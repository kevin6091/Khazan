"""Run checkpointed BigBear animation imports in isolated Editor processes."""
...
BATCH_SIZE = 40
VERSION = "20260910_BigBear_SourceCompositePlaybackV2_RootLockPropagation"
...
def main() -> None:
    ...
    for start in range(0, min(limit, len(ROWS)), BATCH_SIZE):
        ...
        report_path = PROJECT / "Saved/ImportReports/BigBearAnimationBatches" / f"{start:04d}.json"
        ...
        if (
            ...
            continue
        log = PROJECT / "Saved/Logs" / f"BigBearAnimations_{start:04d}_20260910.log"
        stdout = log.with_suffix(".stdout.txt")
        args = [
            str(ENGINE),
            str(PROJECT / "Khazan.uproject"),
            "-run=pythonscript",
            "-script=" + str(PROJECT / "Scripts/Enemies/import_big_bear_animations.py"),
            f"-BigBearStart={start}",
            f"-BigBearCount={count}",
            "-unattended",
            "-nullrhi",
            "-nosound",
            "-nosplash",
        ...
        if process.returncode or data.get("status") != "passed" or len(data.get("assets", [])) != count:
            print("BIG_BEAR_BATCH_FAILED", start, process.returncode, data.get("error"), flush=True)
            raise SystemExit(1)
        reports.append(data)
        print(
            "BIG_BEAR_BATCH_PASSED",
            start,
            count,
            "total",
    ...
    if limit >= len(ROWS):
        ...
        result = {
            ...
            "batch_reports": [
                str(PROJECT / "Saved/ImportReports/BigBearAnimationBatches" / f"{report['start']:04d}.json")
                for report in reports
            ],
        }
        target = PROJECT / "Saved/ImportReports/BigBear_AnimationImportAudit_20260910.json"
