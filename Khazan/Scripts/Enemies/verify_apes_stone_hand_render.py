"""Real-RHI shader and saved ApesStoneHandElite catalogue validation."""
...
REPORT = PROJECT / "Saved/ImportReports/ApesStoneHandElite_RenderAudit_20260915.json"
SUMMARY = META / "RenderValidationSummary.json"
MAP_PATH = MANIFEST["destination_root"] + "/Preview/L_EN_ApesStoneHandElite_Catalogue"
...
def capture_preview(report):
    ...
    position = unreal.Vector(1600.0, 325.0, 430.0)
    target = unreal.Vector(0.0, 325.0, 240.0)
    ...
    texture = unreal.RenderingLibrary.create_render_target2d(world, 1200, 1000, unreal.TextureRenderTargetFormat.RTF_RGBA8_SRGB)
    ...
    # Export display PNG in sRGB; linear RGBA8 made the proof image too dark.
    component.capture_scene()
    folder = PROJECT / "Saved/ImportReports"
    unreal.RenderingLibrary.export_render_target(world, texture, str(folder), "ApesStoneHandElite_RenderPreview_20260915.png")
    path = folder / "ApesStoneHandElite_RenderPreview_20260915.png"
    ...
    component.capture_scene()
    unreal.RenderingLibrary.export_render_target(world, texture, str(folder), "ApesStoneHandElite_BaseColorPreview_20260915.png")
    report["base_color_capture"] = str(folder / "ApesStoneHandElite_BaseColorPreview_20260915.png")
    if "-ApesStoneHandEliteDiagnostic" in unreal.SystemLibrary.get_command_line():
        ...
        component.set_editor_property("capture_source", unreal.SceneCaptureSource.SCS_FINAL_COLOR_LDR)
        component.capture_scene()
        unreal.RenderingLibrary.export_render_target(world, texture, str(folder), "ApesStoneHandElite_DiagnosticOpaque_20260915.png")
    report["preview_capture_settings"] = {"status":"Assistant-selected presentation settings; not source gameplay metadata", "resolution":[1200,1000], "fov_degrees":50.0, "camera_cm":[1600,325,430], "target_cm":[0,325,240], "render_target_format":"RTF_RGBA8_SRGB"}
...
def main() -> None:
    ...
    report["capture_context"] = "EditorAfterWarmup" if "capture_apes_stone_hand_editor_preview.py" in command_line else "ImmediateCommandletCapture"
    try:
        ...
        material_paths = [
            MANIFEST["destination_root"] + "/Materials/Masters/M_EN_ApesStoneHandElite_SurfacePreview",
            MANIFEST["destination_root"] + "/Materials/Masters/M_EN_ApesStoneHandElite_EyePreview",
        ...
        for path in material_paths:
            ...
            print("APES_STONE_HAND_SHADER_OK", path, flush=True)
        ...
        if not current_world or current_world.get_path_name().split(".", 1)[0] != MAP_PATH:
            check(level_subsystem.load_level(MAP_PATH), "Unable to reload saved ApesStoneHandElite catalogue")
        actor_map = {
    ...
    print("APES_STONE_HAND_RENDER_AUDIT_PASSED", flush=True)
