...
import pathlib
import struct
import build_apes_stone_hand_import_manifest as source
...
def main():
    ...
    initial_user = {p:h for p,h in baseline["dirty_file_sha256"].items() if "Scripts/Enemies/prepare_apes_stone_hand_sources.py" not in p}
    ...
    source.write(source.PROJECT / "Saved/ImportReports/ApesStoneHandElite_WorkspacePreservation_20260915.json", preservation)
    print("APES_SOURCE_AUDIT_PASSED", len(result), "mesh;", preservation, flush=True)
