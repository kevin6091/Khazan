"""Restore the ApesStoneHandElite enemy render library from cooked metadata and exports.
...
SKELETON_VERSION = "20260915_ApesStoneHandElite_SourceSkeletonV1"
REPORT = PROJECT / "Saved/ImportReports/ApesStoneHandElite_LibraryBuild_20260915.json"
BASELINE = PROJECT / "Saved/ImportReports/ApesStoneHandElite_ProtectedBaseline_20260915.json"
...
def clear_asset_directory(path: str) -> list[str]:
    if not path.startswith(DEST + "/_ImportStaging/"):
        raise RuntimeError("Refuse cleanup outside new ApesStoneHandElite import staging: " + path)
...
def set_common_metadata(asset, source_package: str, role: str) -> None:
    ...
    EAL.set_metadata_tag(asset, "ApesStoneHandEliteLibraryVersion", VERSION)


def new_asset(path: str, cls, factory):
    if EAL.does_asset_exist(path):
        asset = load(path)
        if EAL.get_metadata_tag(asset, "ApesStoneHandEliteLibraryVersion") != VERSION:
            raise RuntimeError("Unrecognized existing ApesStoneHandElite asset " + path)
        return asset, False
...
def import_one(source: str, destination: str, factory):
    if EAL.does_asset_exist(destination):
        ...
        if EAL.get_metadata_tag(asset, "ApesStoneHandEliteLibraryVersion") != VERSION:
            raise RuntimeError("Unrecognized existing ApesStoneHandElite import " + destination)
        return asset, False
...
def import_textures(report: dict) -> dict[str, object]:
    ...
    for index, row in enumerate(MANIFEST["textures"], 1):
        ...
        if index % 10 == 0 or index == len(MANIFEST["textures"]):
            print("APES_STONE_HAND_TEXTURE", index, "/", len(MANIFEST["textures"]), flush=True)
    return mapping
...
def build_master(category: str, texture_map: dict[str, object], report: dict):
    name = "M_EN_ApesStoneHandElite_" + category + "Preview"
    path = f"{DEST}/Materials/Masters/{name}"
    rebuilt = False
    if EAL.does_asset_exist(path):
        existing = load(path)
        if EAL.get_metadata_tag(existing, "ApesStoneHandEliteMaterialAdapterVersion") != MATERIAL_ADAPTER_VERSION:
            if EAL.get_metadata_tag(existing, "ApesStoneHandEliteLibraryVersion") != VERSION:
    ...
    EAL.set_metadata_tag(master, "ApesStoneHandEliteMaterialAdapterVersion", MATERIAL_ADAPTER_VERSION)
...
def build_materials(texture_map: dict[str, object], report: dict) -> dict[str, object]:
    ...
    for row in MANIFEST["materials"]:
        ...
        if "BlendMode" in overrides:
            if overrides["BlendMode"] != "BLEND_Masked":
                raise RuntimeError("New source blend mode requires explicit reconstruction")
            base.set_editor_property("override_blend_mode", True)
            base.set_editor_property("blend_mode", unreal.BlendMode.BLEND_MASKED)
        if "DitheredLODTransition" in overrides:
            base.set_editor_property("override_dithered_lod_transition", True)
            base.set_editor_property("dithered_lod_transition", overrides["DitheredLODTransition"])
...
def import_mesh(row, material_map, report):
    ...
    if EAL.does_asset_exist(destination):
        ...
        if EAL.get_metadata_tag(mesh, "ApesStoneHandEliteLibraryVersion") != VERSION:
            raise RuntimeError("Unrecognized existing ApesStoneHandElite mesh " + destination)
        created = False
    else:
    ...
    for asset in [mesh, skeleton]:
        EAL.set_metadata_tag(asset, "ApesStoneHandEliteSkeletonVersion", SKELETON_VERSION)
    ...
    EAL.set_metadata_tag(mesh, "SourceMeshAliases", json.dumps(row["source_mesh_aliases"]))
    EAL.set_metadata_tag(mesh, "BoneIndexRemapJSON", json.dumps(row["bone_index_remap"]))
    ...
    print("APES_STONE_HAND_MESH_IMPORTED", destination, flush=True)
...
def main() -> None:
    ...
    try:
        ...
        write(REPORT, report)
    print("APES_STONE_HAND_LIBRARY_BUILD_PASSED", flush=True)
