"""Targeted source discovery and usage selection for WildDog.
...
META = ROOT / "Metadata"
EXTERNAL = pathlib.Path.home() / "Desktop/카잔/EnemyExtracts/WildDog_20260915"
DEST = "/Game/_Art/Enemies/Shared/Beasts/WildDog"
LIVE_DESIGN_ROOT = "BBQ/Content/_Kazan_/Design/Monster/Beast/WhiteDog/"
SHARED_DESIGN_ROOT = "BBQ/Content/_Kazan_/Design/Monster/Beast/PicaroonDog/"
DESIGN_ROOTS = (LIVE_DESIGN_ROOT, SHARED_DESIGN_ROOT)
MODEL_ROOT = "BBQ/Content/_Kazan_/Art/Character/CHA_Model/Monster/PicaroonDog/"
RECIPE_ROOT = "BBQ/Content/_Kazan_/Art/Character/CHA_Data/RandomLookInfo/MonsterType/"
RECIPE = RECIPE_ROOT + "CD_RD_M_WildDog_White_001"
CB = LIVE_DESIGN_ROOT + "Base_Setting/CB_WhiteDog"
AP = LIVE_DESIGN_ROOT + "Base_Setting/AP_WhiteDog"
SA = LIVE_DESIGN_ROOT + "Base_Setting/SA_WhiteDog"
...
def ensure_index():
    ...
    result = subprocess.run(
        [str(extraction.DOTNET), str(extraction.DLL), "index", str(INDEX.parent), "WhiteDog", "WildDog", "PicaroonDog"],
        check=False,
    )
    if result.returncode:
        raise RuntimeError(f"WildDog targeted index failed with exit code {result.returncode}")
...
def inspect():
    ...
    requested = {
        ...
        or (p.startswith(MODEL_ROOT + "Model/") and any(token in p.rsplit("/", 1)[-1] for token in ("WildDog", "PicaroonDog")))
        or (p.startswith(RECIPE_ROOT) and any(token in p for token in ("WildDog", "PicaroonDog")))
        or ("/CHA_Material/Material/" in p and any(token in p for token in ("WildDog", "PicaroonDog")))
        or ("/CHA_Data/BoneModInfo/" in p and "Dog" in p)
    }
    objects, failures = extraction.ensure_metadata(requested, "WildDog_InitialUsageMetadata")
...
def usage():
    ...
    if recipe != RECIPE:
        raise RuntimeError(f"CB_WhiteDog recipe changed: {recipe}")
    defaults = cdo(read(META / (recipe + ".json")))["Properties"]
    effective = {**defaults, **component.get("Properties", {})}
    meshes = [package_path(value["AssetPathName"]) for value in effective["HeadMesh"]]
    parts = effective["HeadPartsList"]
    if len(parts) != 1 or len(parts[0]["MaterialVariations"]) != len(meshes):
        raise RuntimeError("WildDog live HeadMesh/HeadPartsList variation alignment changed")
    variants = []
    for material_index, (mesh, materials) in enumerate(zip(meshes, parts[0]["MaterialVariations"])):
        stem = mesh.rsplit("/", 1)[-1]
        suffix = stem.removeprefix("C_M_WildDog") or "1"
    ...
    variants.sort(key=lambda row: row["role"])
    spawns = []
    for level in ["HeinMach", "StormPass"]:
        ...
        for obj in read(path):
            ...
            if obj.get("Type") == "xxSpawnHandler_Character" and "/WhiteDog/" in actor_bp:
    ...
    if len(spawns) != 17 or {row["level"] for row in spawns} != {"StormPass"}:
        raise RuntimeError(f"Expected the 17 live StormPass WhiteDog spawns, found {len(spawns)}")
    write(ROOT / "VariantUsage.json", {"variants": variants, "level_spawns": spawns,
        "selection_basis": "StormPass uses CB_WhiteDog. Its serialized SCS RandomLook component replaces the White recipe arrays with aligned V3/V2/V1 HeadMesh and material entries. All three cache keys remain live outcomes; geometry is deduplicated only after ActorX non-material chunks compare byte-for-byte.",
        "excluded": [
            {"asset": MODEL_ROOT + "Model/C_M_PicaroonDog_EmptyMesh", "reason": "Original animation carrier; the visible WildDog mesh is used directly while component transform metadata is preserved."},
            {"asset": "C_M_PicaroonDog / V2 / V3 and CM_M_PicaroonDog*", "reason": "Normal PicaroonDog recipe is not referenced by any inspected HeinMach/StormPass spawn; this request's live actor is CB_WhiteDog."},
            {"asset": "C_M_Ghost_PicaroonDog / C_M_Ghost_WildDog and ghost materials", "reason": "Ghost recipe is outside the live CB_WhiteDog spawn closure."}]})
...
def discover():
    ...
    def follow(owner, kind, target):
        ...
        if target.startswith(DESIGN_ROOTS):
            return True
        ...
        if target.startswith(MODEL_ROOT):
            ...
            if "/Model/" in target:
                return target in visible_meshes or name.endswith(("_Skeleton", "_Physics", "_AB", "_CtrlRig", "_IKHit", "_BodyHit"))
            return True
    ...
    pending.update(r for row in spawns for r in row["references"] if r.startswith(DESIGN_ROOTS))
    visited, types, edges, excluded, failures = set(), {}, [], set(), []
    round_number = 0
    while pending:
        round_number += 1
        objects, errors = extraction.ensure_metadata(pending, f"WildDog_Closure_{round_number:02d}")
        ...
        print("WILD_DOG_CLOSURE", round_number, len(visited), "next", len(pending), "failures", len(errors), flush=True)
...
def export():
    ...
    extraction.run_exporter("export", ROOT / "Assets", closure["exports"], "WildDog_ArtAnimationExport")
    extraction.run_exporter("raw", EXTERNAL / "RawCookedArchive", closure["packages"], "WildDog_RawArchive")
    missing = []
    for package in closure["exports"]:
        if not any((ROOT / "Assets" / f"{package}{suffix}").exists() for suffix in (".psk", ".pskx", ".psa", ".png")):
            missing.append(package)
    if missing:
        raise RuntimeError(f"Missing converted WildDog exports: {missing}")
