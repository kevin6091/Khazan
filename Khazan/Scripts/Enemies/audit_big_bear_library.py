"""Fresh-process audit and preview catalogue for the shared BigBear library."""
...
CONTENT_ROOT = PROJECT / "Content/_Art/Enemies/Shared/Beasts/BigBear"
META = CONTENT_ROOT / "Metadata/Extraction_20260910"
...
ANIMATION_IMPORT_AUDIT = json.loads(
    (PROJECT / "Saved/ImportReports/BigBear_AnimationImportAudit_20260910.json").read_text(encoding="utf-8")
)
...
MAP_PATH = DEST + "/Preview/L_EN_BigBear_Catalogue"
REPORT = PROJECT / "Saved/ImportReports/BigBear_FreshAudit_20260910.json"
SUMMARY = META / "ValidationSummary.json"
TIMING_VERSION = "20260910_BigBear_SourceCompositePlaybackV2_RootLockPropagation"
ASSEMBLY_VERSION = "20260910_BigBearVisualAssemblyV1"
...
def build_catalogue(report: dict) -> None:
    if EAL.does_asset_exist(MAP_PATH):
        check(LEVELS.load_level(MAP_PATH), "Unable to load BigBear catalogue")
    else:
        check(LEVELS.new_level(MAP_PATH), "Unable to create BigBear catalogue")
    ...
    for index, row in enumerate(CATALOG):
        ...
        if actor is None:
            actor = ACTORS.spawn_actor_from_class(
                ...
                unreal.Vector(0.0, (index - 1) * spacing_cm, 0.0),
            )
        check(actor is not None, "Unable to place " + label)
        actor.set_actor_label(label)
        actor.set_folder_path("EnemyCatalogue/BigBear")

    if "BigBearCatalogue_KeyLight" not in existing:
        key = ACTORS.spawn_actor_from_class(
            unreal.DirectionalLight,
            unreal.Vector(-300.0, 0.0, 700.0),
            unreal.Rotator(pitch=-40.0, yaw=-35.0, roll=0.0),
        )
        key.set_actor_label("BigBearCatalogue_KeyLight")
        ...
        fill.set_actor_label("BigBearCatalogue_FillLight")
    ...
    check(LEVELS.save_current_level(), "Unable to save BigBear catalogue")
...
def verify_materials(report: dict) -> None:
    ...
    master_paths = {
        "Surface": DEST + "/Materials/Masters/M_EN_BigBear_SurfacePreview",
        "Eye": DEST + "/Materials/Masters/M_EN_BigBear_EyePreview",
    }
    for path in master_paths.values():
        master = load(path)
        check(isinstance(master, unreal.Material), "Preview master type mismatch " + path)
        check(EAL.get_metadata_tag(master, "BigBearMaterialAdapterVersion") == MATERIAL_ADAPTER_VERSION, "Master version " + path)
        check(
    ...
    for row in MANIFEST["materials"]:
        ...
        check(asset_path(material.get_editor_property("parent")) == master_paths[row["category"]], "Material parent mismatch " + row["destination"])
        ...
        check(json.loads(EAL.get_metadata_tag(material, "OriginalPropertiesJSON")) == properties, "Material metadata mismatch " + row["destination"])
        for value in properties.get("ScalarParameterValues", []):
            ...
            check(abs(actual - float(value["ParameterValue"])) <= 1e-6, "Scalar mismatch " + row["destination"] + " " + name)
            scalar_parameters += 1
        for value in properties.get("VectorParameterValues", []):
            ...
            check(error <= 1e-6, "Vector mismatch " + row["destination"] + " " + name)
            vector_parameters += 1
...
def verify_mesh(report: dict):
    row = MANIFEST["body"]
    mesh = load(MESH_PATH)
    skeleton = load(SKELETON_PATH)
    check(mesh.get_editor_property("skeleton") == skeleton, "BigBear mesh skeleton mismatch")
    slots = list(mesh.get_editor_property("materials"))
    check([str(slot.material_slot_name) for slot in slots] == row["source_slot_names"], "BigBear slot-name mismatch")
    material_paths = {material["source_package"]: material["destination"] for material in MANIFEST["materials"]}
    check(
        [asset_path(slot.material_interface) for slot in slots] == [material_paths[path] for path in row["materials"]],
        "BigBear mesh default material mismatch",
    )
    ...
    check(len(bones) == row["animation_reference_bones"] == 79, "BigBear reference-bone count mismatch")
    check(not HELPER_BONES.difference(bones), "BigBear helper bones missing")
    check(MESH_EDITOR.get_num_verts(mesh, 0) == 30023, "BigBear LOD0 vertex mismatch")
    check(MESH_EDITOR.get_lod_count(mesh) == 1, "Unexpected project LOD count")
    check(EAL.get_metadata_tag(mesh, "SourceLODCount") == str(row["source_lod_count"]), "Source LOD metadata mismatch")
    check(EAL.get_metadata_tag(mesh, "ImportedLODCount") == "1", "Imported LOD metadata mismatch")
    source_files = [pathlib.Path(row["original_source_file"]), pathlib.Path(row["source_file"])] + [
        pathlib.Path(path) for path in row["lod_files"]
    ]
    for source_file in source_files:
        check(source_file.is_file() and source_file.stat().st_size > 0, "Missing archived mesh file " + str(source_file))
    report["mesh"] = {
        "asset": MESH_PATH,
        "skeleton": SKELETON_PATH,
        "reference_bones": len(bones),
        "animation_helper_bones": sorted(HELPER_BONES),
        "lod0_vertices": MESH_EDITOR.get_num_verts(mesh, 0),
        "source_lods_archived": row["source_lod_count"],
        "project_lods": MESH_EDITOR.get_lod_count(mesh),
        "source_file_sha256": {str(path): sha256(path) for path in source_files},
    }
...
def verify_animations(report: dict, mesh, skeleton) -> None:
    ...
    check(ANIMATION_IMPORT_AUDIT["assets"] == len(ANIMATIONS) == 102, "Animation count mismatch")
    ...
    for index, row in enumerate(ANIMATIONS, 1):
        ...
        if index % 20 == 0 or index == len(ANIMATIONS):
            print("BIG_BEAR_AUDIT_ANIMATION", index, "/", len(ANIMATIONS), flush=True)
...
def verify_assemblies(report: dict, mesh) -> None:
    check(len(CATALOG) == len(MANIFEST["material_variants"]) == 3, "Assembly variant count mismatch")
    for catalog_row, source_row in zip(CATALOG, MANIFEST["material_variants"]):
        ...
        check(EAL.get_metadata_tag(blueprint, "EnemySpecies") == "BigBear", "Blueprint species mismatch")
        check(EAL.get_metadata_tag(blueprint, "BigBearAssemblyVersion") == ASSEMBLY_VERSION, "Blueprint version mismatch")
        ...
        check(selection["level_context"].keys() == {"HeinMach", "StormPass"}, "Blueprint level provenance mismatch")
    ...
    for row in MANIFEST["material_variants"]:
        ...
        check(len(components) == 1, "Visual BigBear BP must have exactly one skeletal component " + label)
        component = components[0]
        check(asset_path(component.get_skinned_asset()) == MESH_PATH, "Blueprint mesh mismatch " + label)
        check(component.get_num_bones() == 79, "Blueprint bone count mismatch " + label)
        check(
...
def verify_structure(report: dict) -> None:
    expected = {
        ...
        SKELETON_PATH,
        MAP_PATH,
        DEST + "/Materials/Masters/M_EN_BigBear_SurfacePreview",
        DEST + "/Materials/Masters/M_EN_BigBear_EyePreview",
    }
    ...
    check(actual == expected, "BigBear root contains missing or extra UE assets")
    orphan_files = [
...
def main() -> None:
    ...
    print("BIG_BEAR_FRESH_AUDIT_PASSED", flush=True)
