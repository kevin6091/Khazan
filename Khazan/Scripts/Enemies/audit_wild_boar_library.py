"""Fresh-process audit and preview catalogue for the shared WildBoar library."""
...
CONTENT_ROOT = PROJECT / "Content/_Art/Enemies/Shared/Beasts/WildBoar"
...
CATALOG = json.loads((META / "WildBoarAssemblyCatalog.json").read_text(encoding="utf-8"))
EVENTS = json.loads((META / "PlaybackEventTimes.json").read_text(encoding="utf-8"))["events"]
ANIMATION_IMPORT_AUDIT = json.loads(
    (PROJECT / "Saved/ImportReports/WildBoar_AnimationImportAudit_20260915.json").read_text(encoding="utf-8")
)
...
MAP_PATH = DEST + "/Preview/L_EN_WildBoar_Catalogue"
REPORT = PROJECT / "Saved/ImportReports/WildBoar_FreshAudit_20260915.json"
SUMMARY = META / "ValidationSummary.json"
TIMING_VERSION = "20260915_WildBoar_SourceCompositePlaybackV2_RootLockPropagation"
ASSEMBLY_VERSION = "20260915_WildBoarVisualAssemblyV1"
MATERIAL_ADAPTER_VERSION = "20260915_WildBoarPreviewV4_BoundedParameterRetention"
...
def build_catalogue(report: dict) -> None:
    if EAL.does_asset_exist(MAP_PATH):
        check(LEVELS.load_level(MAP_PATH), "Unable to load WildBoar catalogue")
    else:
        check(LEVELS.new_level(MAP_PATH), "Unable to create WildBoar catalogue")
    ...
    for index, row in enumerate(CATALOG):
        ...
        actor.set_actor_location(unreal.Vector(0.0, index * spacing_cm, 0.0), False, False)
        actor.set_actor_label(label)
        actor.set_folder_path("EnemyCatalogue/WildBoar")

    if "WildBoarCatalogue_KeyLight" not in existing:
        key = ACTORS.spawn_actor_from_class(
            unreal.DirectionalLight,
            unreal.Vector(-300.0, 0.0, 700.0),
            unreal.Rotator(pitch=-40.0, yaw=-35.0, roll=0.0),
        )
        key.set_actor_label("WildBoarCatalogue_KeyLight")
        ...
        fill.set_actor_label("WildBoarCatalogue_FillLight")
        ...
        floor.set_actor_label("WildBoarCatalogue_Floor")
        ...
        camera = ACTORS.spawn_actor_from_class(
            unreal.CameraActor,
            unreal.Vector(-1200.0, spacing_cm, 300.0),
            unreal.Rotator(pitch=0.0, yaw=0.0, roll=0.0),
        )
        camera.set_actor_label("WildBoarCatalogue_Camera")
    ...
    check(LEVELS.save_current_level(), "Unable to save WildBoar catalogue")
...
def verify_materials(report: dict) -> None:
    ...
    master_paths = {
        "Surface": DEST + "/Materials/Masters/M_EN_WildBoar_SurfacePreview",
        "Eye": DEST + "/Materials/Masters/M_EN_WildBoar_EyePreview",
    }
    for path in master_paths.values():
        master = load(path)
        check(isinstance(master, unreal.Material), "Preview master type mismatch " + path)
        check(EAL.get_metadata_tag(master, "WildBoarMaterialAdapterVersion") == MATERIAL_ADAPTER_VERSION, "Master version " + path)
        check(
...
def verify_animations(report: dict, mesh, skeleton) -> None:
    ...
    for index, row in enumerate(ANIMATIONS, 1):
        ...
        if index % 20 == 0 or index == len(ANIMATIONS):
            print("WILD_BOAR_AUDIT_ANIMATION", index, "/", len(ANIMATIONS), flush=True)
...
def verify_assemblies(report: dict, mesh) -> None:
    ...
    for catalog_row, source_row in zip(CATALOG, MANIFEST["material_variants"]):
        ...
        check(EAL.get_metadata_tag(blueprint, "EnemySpecies") == "WildBoar", "Blueprint species mismatch")
        check(EAL.get_metadata_tag(blueprint, "WildBoarAssemblyVersion") == ASSEMBLY_VERSION, "Blueprint version mismatch")
        ...
        check(set(selection["level_context"]) == {"StormPass"}, "Blueprint level provenance mismatch")
    ...
    for row in MANIFEST["material_variants"]:
        ...
        check(len(components) == 1, "Visual WildBoar BP must have exactly one skeletal component " + label)
...
def verify_structure(report: dict) -> None:
    expected = {
        ...
        SKELETON_PATH,
        MAP_PATH,
        DEST + "/Materials/Masters/M_EN_WildBoar_SurfacePreview",
        DEST + "/Materials/Masters/M_EN_WildBoar_EyePreview",
    }
    ...
    check(actual == expected, "WildBoar asset set mismatch: missing=" + repr(expected-actual) + "; extra=" + repr(actual-expected))
    orphan_files = [
...
def main() -> None:
    ...
    print("WILD_BOAR_FRESH_AUDIT_PASSED", flush=True)
