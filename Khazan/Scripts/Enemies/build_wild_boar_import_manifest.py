"""Create the canonical UE import manifest for live StormPass WildBoar looks."""
...
PROJECT_META = PROJECT / "Content/_Art/Enemies/Shared/Beasts/WildBoar/Metadata/Extraction_20260915"

CB = "BBQ/Content/_Kazan_/Design/Monster/Beast/WildBoar_New/Base_Setting/CB_WildBoar_New"
AP = "BBQ/Content/_Kazan_/Design/Monster/Beast/WildBoar_New/Base_Setting/AP_WildBoar_New"
BODY = "BBQ/Content/_Kazan_/Art/Character/CHA_Model/Monster/WildBoar/Model/C_M_WildBoar"
...
def derive_full_animation_skeleton_psk(body_file: pathlib.Path, animation_files: list[pathlib.Path]):
    ...
    if len(layouts) != 1:
        raise RuntimeError(f"WildBoar animation skeleton layouts differ: {[len(value) for value in layouts]}")
...
def main():
    ...
    for variant in selected_variants:
        ...
        mesh_rows.append({
            ...
            "skeleton_destination": f"{DEST}/Skeletons/SKEL_EN_WildBoar",
    ...
    for p, ts in sorted(types.items()):
        for kind, target in [("AnimSequence", source_anims), ("AnimComposite", composites)]:
            ...
            if skeleton not in skeleton_mesh:
                excluded.append({"source": p, "skeleton": skeleton, "reason": "Referenced animation uses another character or helper skeleton; retain metadata without importing it into the WildBoar skeleton."})
                continue
    ...
    for p, obj in hierarchy.items():
        ...
        rows.append({
            "source_package": p, "destination": f"{DEST}/Materials/Instances/MI_EN_WildBoar_" + p.rsplit("/", 1)[1],
    ...
    for p in sorted(required_textures):
        ...
        textures.append({
            ...
            "destination": f"{DEST}/Textures/" + unique_name("T_EN_WildBoar_", p, tex_names),
    ...
    for index, selection in enumerate(selected_variants):
        ...
        variants.append({
            ...
            "blueprint": DEST + "/Blueprints/BP_EN_WildBoar_" + selection["role"], "mesh": mesh["destination"],
    ...
    result = {
        "schema_version": 1, "library_version": "20260915_WildBoarV1", "destination_root": DEST,
