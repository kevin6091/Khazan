...
import pathlib
import struct
import build_wild_boar_import_manifest as source
...
def main():
    ...
    initial_user = {p:h for p,h in baseline["dirty_file_sha256"].items() if "Scripts/Enemies/prepare_wild_boar_sources.py" not in p}
    ...
    source.write(source.PROJECT / "Saved/ImportReports/WildBoar_WorkspacePreservation_20260915.json", preservation)
    print("WILD_BOAR_SOURCE_AUDIT_PASSED", len(result), "mesh;", preservation, flush=True)
