"""Real-RHI shader and saved WildDog catalogue validation."""
...
REPORT = PROJECT / "Saved/ImportReports/WildDog_RenderAudit_20260915.json"
SUMMARY = META / "RenderValidationSummary.json"
MAP_PATH = MANIFEST["destination_root"] + "/Preview/L_EN_WildDog_Catalogue"
...
def capture_preview(report):
    ...
    position = unreal.Vector(1000.0, 400.0, 250.0)
    target = unreal.Vector(0.0, 400.0, 90.0)
    ...
    unreal.RenderingLibrary.export_render_target(world, texture, str(folder), "WildDog_RenderPreview_20260915.png")
    path = folder / "WildDog_RenderPreview_20260915.png"
    ...
    component.capture_scene()
    unreal.RenderingLibrary.export_render_target(world, texture, str(folder), "WildDog_BaseColorPreview_20260915.png")
    report["base_color_capture"] = str(folder / "WildDog_BaseColorPreview_20260915.png")
    if "-WildDogDiagnostic" in unreal.SystemLibrary.get_command_line():
        ...
        component.set_editor_property("capture_source", unreal.SceneCaptureSource.SCS_FINAL_COLOR_LDR)
        component.capture_scene()
        unreal.RenderingLibrary.export_render_target(world, texture, str(folder), "WildDog_DiagnosticOpaque_20260915.png")
    report["preview_capture_settings"] = {"status":"Assistant-selected presentation settings; not source gameplay metadata", "resolution":[1200,1000], "fov_degrees":55.0, "camera_cm":[1000,400,250], "target_cm":[0,400,90], "render_target_format":"RTF_RGBA8_SRGB"}
...
def main() -> None:
    ...
    report["capture_context"] = "EditorAfterWarmup" if "capture_wild_dog_editor_preview.py" in command_line else "ImmediateCommandletCapture"
    try:
        ...
        material_paths = [
            MANIFEST["destination_root"] + "/Materials/Masters/M_EN_WildDog_SurfacePreview",
            MANIFEST["destination_root"] + "/Materials/Masters/M_EN_WildDog_EyePreview",
        ...
        for path in material_paths:
            ...
            print("WILD_DOG_SHADER_OK", path, flush=True)
        ...
        if not current_world or current_world.get_path_name().split(".", 1)[0] != MAP_PATH:
            check(level_subsystem.load_level(MAP_PATH), "Unable to reload saved WildDog catalogue")
        actor_map = {
    ...
    print("WILD_DOG_RENDER_AUDIT_PASSED", flush=True)
