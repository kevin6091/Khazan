...
void AKZPlayer::HandleInputMove(const FVector2D& MovementInput, const FRotator& ControlRotation)
{
...
void AKZPlayer::HandleInputMoveReleased()
{
...
void AKZPlayer::HandleInputSprint()
{
...
void AKZPlayer::HandleInputSprintReleased()
{
...
void AKZPlayer::HandleInputSprintCanceled()
{
...
void AKZPlayer::RefreshRequestedGait()
{
	...
	// 그것보다 클 때만 Run을 요청한다.
	const EKZGait StickGait = Intent.InputAmount > RunInputThreshold ? EKZGait::Run : EKZGait::Walk;

	// Sprint 입력 요청이 있으면 StickGait보다 Sprint 요청이 우선한다.
	// 이 값은 아직 Constraint가 적용되지 않은 RequestedGait다.
	const EKZGait RequestedGait = bSprintRequested 	? EKZGait::Sprint : StickGait;
