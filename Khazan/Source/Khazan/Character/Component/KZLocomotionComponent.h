...
#include "KZLocomotionComponent.generated.h"
 ...
 */
struct KHAZAN_API FKZLocomotionIntentHandle
{
public:
...
private:
	...
	friend class UKZLocomotionComponent;

	// handle을 발급하고 활성 여부를 판정하는 Component의 약한 참조다. Character의 ASC Owner와는 무관하다.
	TWeakObjectPtr<UKZLocomotionComponent> Owner;
...
struct KHAZAN_API FKZMovementConstraintHandle
{
	GENERATED_BODY()
...
DECLARE_MULTICAST_DELEGATE_OneParam(FKZMovementInputPermissionChanged, bool /* bMovementAllowed */);

// Tick 없이 config, intent, constraint, tag 변화에 반응해 이동 정책을 계산하는 공통 컴포넌트다.
UCLASS(ClassGroup = (KZ), meta = (BlueprintSpawnableComponent))
class KHAZAN_API UKZLocomotionComponent : public UActorComponent
{
	GENERATED_BODY()

public:
	UKZLocomotionComponent();

	// CharacterDefinition에서 받은 설정을 검증하고 이 Pawn 수명의 런타임 복사본으로 보관한다.
	// 활성 intent source나 constraint가 존재하는 동안에는 기준 설정 교체를 거절한다.
	bool InitializeMovementConfig(const FKZLocomotionConfig& InConfig);
	...
	FKZLocomotionIntentHandle BeginLocomotionIntentSource(UObject* Source, EKZLocomotionIntentSource SourceType);
	...
	void EndLocomotionIntentSource(FKZLocomotionIntentHandle& Handle);

	// ItentHandle이 현재 활성화된 Handle인지 확인
	bool IsActiveLocomotionIntentHandle(const FKZLocomotionIntentHandle& Handle) const;

	// 유효한 token을 가진 Player 입력 어댑터 또는 AI 이동 경로가 월드 평면 방향과 세기를 기록한다.
	// 이동 허용 여부와 무관하게 raw intent를 보존하며, stale handle이면 false를 반환한다.
	bool SetMoveInputWorld(const FKZLocomotionIntentHandle& Handle, const FVector& Input);

	// 실제 입력 Released, 경로 종료, UnPossess에서 raw 방향과 세기만 0으로 만든다.
	bool ClearMoveInput(const FKZLocomotionIntentHandle& Handle);

	// 현재 source가 원하는 Walk/Run/Sprint를 기록하고 최종 Policy를 다시 계산한다.
	bool SetRequestedGait(const FKZLocomotionIntentHandle& Handle, EKZGait Gait);

	// source의 요청 gait를 CharacterDefinition의 DefaultRequestedGait로 되돌리고 Policy를 다시 계산한다.
	bool ResetRequestedGaitToDefault(const FKZLocomotionIntentHandle& Handle);

	// Controller가 원하는 회전 모드를 기록한다. Ability 등이 회전을 강제할 때는 Constraint의 override를 사용한다.
	bool SetRequestedRotationMode(const FKZLocomotionIntentHandle& Handle, EKZRotationMode Mode);
	...
	UFUNCTION(BlueprintCallable, Category = "KZ|Locomotion")
	FKZMovementConstraintHandle AcquireMovementConstraint(UObject* Source, const FKZMovementConstraint& Constraint);
	...
	UFUNCTION(BlueprintCallable, Category = "KZ|Locomotion")
	bool ReleaseMovementConstraint(UPARAM(ref) FKZMovementConstraintHandle& Handle);
	...
	const FKZLocomotionIntent& GetLocomotionIntent() const
	{
		return Intent;
	}

	// C++ Game Thread에서 즉시 읽기 위한 파생 Policy의 const 참조다. 외부에서 수정할 수 없다.
	const FKZResolvedMovementPolicy& GetResolvedMovementPolicy() const
	{
		return ResolvedPolicy;
	}

	// Blueprint/Probe가 원본을 수정하지 않고 관측하도록 raw Intent 복사본을 반환한다.
	UFUNCTION(BlueprintPure, Category = "KZ|Locomotion")
	FKZLocomotionIntent GetLocomotionIntentSnapshot() const
	{
		return Intent;
	}
	...
	UFUNCTION(BlueprintPure, Category = "KZ|Locomotion")
	FKZResolvedMovementPolicy GetResolvedMovementPolicySnapshot() const
	{
		return ResolvedPolicy;
	}

	// 기존 C++ 소비자를 위한 편의 조회다. 별도 계산 없이 Policy에 저장된 최종 gait를 반환한다.
	EKZGait GetResolvedGait() const
	{
		return ResolvedPolicy.ResolvedGait;
	}

	// native 구독자가 callback을 등록·해제할 수 있도록 이 Component가 소유한 delegate를 반환한다.
	FKZMovementInputPermissionChanged& OnMovementInputPermissionChanged()
	{
...
private:
	...
	FKZMovementInputPermissionChanged MovementInputPermissionChanged;

	// CharacterDefinition에서 검증 후 복사한 이 Pawn의 읽기 전용 이동 기준값이다.
	FKZLocomotionConfig MovementConfig;
	...
	UPROPERTY(Transient, BlueprintReadOnly, Category = "Locomotion", meta = (AllowPrivateAccess = "true"))
	FKZResolvedMovementPolicy ResolvedPolicy;
	...
	EKZLocomotionIntentSource ActiveIntentSourceType = EKZLocomotionIntentSource::None;
