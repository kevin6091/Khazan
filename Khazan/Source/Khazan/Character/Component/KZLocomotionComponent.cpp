...
namespace
{
	...
	EKZGait GetMoreRestrictiveGait(const EKZGait A, const EKZGait B)
	{
		const int32 RankA = KZLocomotion::GetGaitRestrictionRank(A);

		const int32 RankB = KZLocomotion::GetGaitRestrictionRank(B);
...
UKZLocomotionComponent::UKZLocomotionComponent()
{
	PrimaryComponentTick.bCanEverTick = false;
}

void UKZLocomotionComponent::BeginPlay()
{
	Super::BeginPlay();
	...
	FOnGameplayEffectTagCountChanged& Event =
		ASC->RegisterGameplayTagEvent(KZGameplayTags::Block_Movement_Input, EGameplayTagEventType::NewOrRemoved);

	// Block 태그를 새로 부여하는 코드가 아니라, 태그 경계 변화 callback 한 건을 구독하는 코드다.
	MovementBlockChangedHandle = Event.AddUObject(this,&UKZLocomotionComponent::HandleMovementBlockChanged);

	// 구독 전부터 Effect가 적용됐을 수 있으므로 현재 ASC count를 즉시 다시 읽는다.
	RefreshMovementPermission();
}

bool UKZLocomotionComponent::InitializeMovementConfig(const FKZLocomotionConfig& InConfig)
{
	...
	if (!InConfig.IsValid(Error))
	{
		...
		MovementConfig = FKZLocomotionConfig{};
		Intent = FKZLocomotionIntent{};
		ResolvedPolicy = FKZResolvedMovementPolicy{};
...
void UKZLocomotionComponent::ResetIntentToConfigDefaults()
{
...
// IntentHandle 발급
FKZLocomotionIntentHandle UKZLocomotionComponent::BeginLocomotionIntentSource(UObject* Source,
	EKZLocomotionIntentSource SourceType)
{
	check(IsInGameThread());

	// 기본 생성된 Handle은 Owner = null, Id = invalid인 실패 반환값.
	FKZLocomotionIntentHandle Handle;

	// Config가 준비되지 않았거나, 실제 Source가 InValid거나, SourceType이 None이면 쓰기 권한을 발급하지 않음.
	if (!bHasValidMovementConfig || !IsValid(Source) || SourceType == EKZLocomotionIntentSource::None)
	{
...
void UKZLocomotionComponent::EndLocomotionIntentSource(FKZLocomotionIntentHandle& Handle)
{
	check(IsInGameThread());
	...
	ActiveIntentSourceType = EKZLocomotionIntentSource::None;
...
bool UKZLocomotionComponent::IsActiveLocomotionIntentHandle(const FKZLocomotionIntentHandle& Handle) const
{
	check(IsInGameThread());
...
bool UKZLocomotionComponent::SetMoveInputWorld(const FKZLocomotionIntentHandle& Handle, const FVector& Input)
{
	check(IsInGameThread());
...
bool UKZLocomotionComponent::ClearMoveInput(const FKZLocomotionIntentHandle& Handle)
{
	check(IsInGameThread());
...
bool UKZLocomotionComponent::SetRequestedGait(const FKZLocomotionIntentHandle& Handle, EKZGait Gait)
{
	check(IsInGameThread());

	// 현재 token과 지원하는 Gait 값이 모두 유효해야 한다.
	if (!IsActiveLocomotionIntentHandle(Handle) ||
		!KZLocomotion::IsSupportedGait(Gait))
	{
		return false;
	}
...
bool UKZLocomotionComponent::ResetRequestedGaitToDefault(const FKZLocomotionIntentHandle& Handle)
{
	check(IsInGameThread());
...
bool UKZLocomotionComponent::SetRequestedRotationMode(const FKZLocomotionIntentHandle& Handle, EKZRotationMode Mode)
{
	check(IsInGameThread());

	// 현재 token과 지원하는 회전 enum이 모두 유효해야 한다.
	if (!IsActiveLocomotionIntentHandle(Handle) ||
		!KZLocomotion::IsSupportedRotationMode(Mode))
	{
		return false;
	}
...
FKZMovementConstraintHandle UKZLocomotionComponent::AcquireMovementConstraint(UObject* Source,
	const FKZMovementConstraint& Constraint)
{
	check(IsInGameThread());

	FKZMovementConstraintHandle Handle;

	const bool bIsGaitSupported = KZLocomotion::IsSupportedGait(Constraint.MaxAllowedGait);
	...
	const bool bIsRotationModeSupported = !Constraint.bOverrideRotationMode ||
		KZLocomotion::IsSupportedRotationMode(Constraint.RotationModeOverride);
...
bool UKZLocomotionComponent::ReleaseMovementConstraint(FKZMovementConstraintHandle& Handle)
{
	check(IsInGameThread());
...
void UKZLocomotionComponent::RebuildAndApplyMovementPolicy()
{
	check(IsInGameThread());
	...
	FKZResolvedMovementPolicy NewPolicy;
	...
	EKZGait MaxAllowedGait = MovementConfig.DefaultMaxAllowedGait;
	EKZRotationMode ResolvedRotationMode = Intent.RequestedRotationMode;
	...
	for (const TPair<FGuid, FActiveMovementConstraint>& Pair : ActiveConstraints)
	{
		...
		const FKZMovementConstraint& Constraint = ActiveConstraint.Constraint;
...
void UKZLocomotionComponent::ApplyMovementPolicyToCharacter()
{
	check(IsInGameThread());

	// 이 Component가 속한 KZ Character와 그 CMC를 찾는다.
	AKZCharacter* Character = Cast<AKZCharacter>(GetOwner());
...
void UKZLocomotionComponent::HandleMovementBlockChanged(const FGameplayTag Tag, int32 NewCount)
{
	// 인자를 사용하지 않음.
...
void UKZLocomotionComponent::RefreshMovementPermission()
{
	check(IsInGameThread());
	...
	const bool bIsMovementAllowedByTags = ASC && ASC->GetTagCount(KZGameplayTags::Block_Movement_Input) == 0;
...
bool UKZLocomotionComponent::IsMovementInputAllowed() const
{
	check(IsInGameThread());
...
void UKZLocomotionComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	if (UAbilitySystemComponent* ASC = ObservedAbilitySystemComponent.Get())
	{
		if (MovementBlockChangedHandle.IsValid())
		{
			ASC->UnregisterGameplayTagEvent(
				...
				KZGameplayTags::Block_Movement_Input,
